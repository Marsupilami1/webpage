...
void BuildStaticPages(ssg::Renderer &renderer) {
    ...
    if (std::filesystem::exists(build_path)) {
        ...
        if (ec)
            throw std::runtime_error("Failed to backup old build: " +
                                     ec.message());
    }
...
auto main(int argc, char *argv[]) -> int {
    ...
    auto description = std::make_shared<crow::mustache::template_t>(
        crow::mustache::load("description.mustache"));
    renderer.AddComponent("Description", description);
    auto article = std::make_shared<crow::mustache::template_t>(
        crow::mustache::load("article.mustache"));
    renderer.AddComponent("Article", article);
    auto header = std::make_shared<crow::mustache::template_t>(
        crow::mustache::load("header.mustache"));
    renderer.AddComponent("Header", header);
